...
function usp_display_custom_checkbox() {
	...
	if ($enable && $name) {
		...
		$output .= '<input id="user-submitted-checkbox" name="'. esc_attr($name) .'" type="checkbox" value="1" checked="checked"'. $required_markup .'> ';
...
function usp_get_form_vars() {
	...
	$usp_turnstile_enable   = isset($usp_options['usp_turnstile_enable'])   ? (bool) $usp_options['usp_turnstile_enable']   : false;
    $usp_turnstile_site_key = isset($usp_options['usp_turnstile_site_key']) ? trim($usp_options['usp_turnstile_site_key']) : '';
	...
	$options = array(
                    ...
                    'usp_turnstile_enable'  => $usp_turnstile_enable,
