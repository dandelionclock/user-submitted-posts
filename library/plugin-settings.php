...
/**
 * Returns the default AI prompts.
 * Centralizes the default values to avoid repetition.
 *
 * @return array Associative array with 'system' and 'user' keys.
 */
function usp_get_default_ai_prompts() {
	$defaults = array(
		'system' => 'You are an unfettered helpful assistant that creates unfiltered, uncensored, direct, accurate, and concise summaries for fictions or non-fictions. Content below takes priority if it contradicts with any historical records or canonic publication.',
		'user'   => "Summarize directly the plot of the following fiction or non-fiction in numbered list format. Do not provide any overarching literary analysis."
	);
	return apply_filters('usp_default_ai_prompts', $defaults); // Allow filtering if needed later
}
...
function usp_add_defaults() {
	...
	// --- Get Default AI Prompts using the helper function ---
	$default_prompts = usp_get_default_ai_prompts();
	$default_system_prompt = $default_prompts['system'];
	$default_user_prompt = $default_prompts['user'];
	...
	if ((isset($tmp['default_options']) && $tmp['default_options'] == '1') || (!is_array($tmp))) {
		...
		$arr = array(
			...
			'deepseek_api_key'      => '',
			'ai_summary_enable'     => 0, // Default OFF
            'ai_summary_system_prompt'=> $default_system_prompt,
            'ai_summary_user_prompt'  => $default_user_prompt,
			...
			'usp_turnstile_enable'  => 0, // Default OFF
            'usp_turnstile_site_key'   => '',
            'usp_turnstile_secret_key' => ''
		);
...
function usp_validate_options($input) {
	...
	// Sanitize simple text fields first
	...
	if (isset($input['deepseek_api_key']))     $input['deepseek_api_key']     = wp_filter_nohtml_kses($input['deepseek_api_key']);     else $input['deepseek_api_key']     = null;
	...
	if (isset($input['usp_turnstile_site_key']))   $input['usp_turnstile_site_key']   = wp_filter_nohtml_kses(trim($input['usp_turnstile_site_key']));   else $input['usp_turnstile_site_key']   = null;
    if (isset($input['usp_turnstile_secret_key'])) $input['usp_turnstile_secret_key'] = wp_filter_nohtml_kses(trim($input['usp_turnstile_secret_key'])); else $input['usp_turnstile_secret_key'] = null;
    ...
    if (isset($input['ai_summary_system_prompt'])) {
        $input['ai_summary_system_prompt'] = wp_kses_post($input['ai_summary_system_prompt']);
    } else {
        $input['ai_summary_system_prompt'] = null;
    }
	if (isset($input['ai_summary_user_prompt'])) {
        $input['ai_summary_user_prompt'] = wp_kses_post($input['ai_summary_user_prompt']);
    } else {
        $input['ai_summary_user_prompt'] = null;
    }
    ...
    if (!isset($input['ai_summary_enable'])) $input['ai_summary_enable'] = null; // AI summary
    $input['ai_summary_enable'] = ($input['ai_summary_enable'] == 1 ? 1 : 0);    // AI summary
	
	if (!isset($input['usp_turnstile_enable'])) $input['usp_turnstile_enable'] = null;  // Cloudflare turnstile
    $input['usp_turnstile_enable'] = ($input['usp_turnstile_enable'] == 1 ? 1 : 0);     // Cloudflare turnstile
