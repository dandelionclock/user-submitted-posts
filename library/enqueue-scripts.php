...
/**
 * Adds async and defer attributes to the Turnstile script tag.
 *
 * @param string $tag    The <script> tag for the enqueued script.
 * @param string $handle The script's registered handle.
 * @return string Modified script tag.
 */
function usp_add_async_defer_to_turnstile($tag, $handle) {
    // Check if it's our specific script handle
    if ('usp_turnstile' === $handle) {
        // Add async and defer attributes just before the closing >
        // Ensure src attribute exists to avoid potential issues (though unlikely)
...
// Add the filter to modify the script tag output
add_filter('script_loader_tag', 'usp_add_async_defer_to_turnstile', 10, 2);
...
function usp_enqueueResources() {
	...
	if ($display_js) {
        ...
        wp_enqueue_script('usp_turnstile', 'https://challenges.cloudflare.com/turnstile/v0/api.js', array(), null, false); // false = load in header
