...
function usp_render_form() {
	...
	// --- Get Default AI Prompts using the helper function for placeholders ---
	$default_prompts = usp_get_default_ai_prompts();
	$default_system_placeholder = $default_prompts['system'];
	$default_user_placeholder = $default_prompts['user'];
	
	// Get current saved values
	$system_prompt_value = isset($usp_options['ai_summary_system_prompt']) ? $usp_options['ai_summary_system_prompt'] : '';
	$user_prompt_value   = isset($usp_options['ai_summary_user_prompt']) ? $usp_options['ai_summary_user_prompt'] : '';
				...
				<div class="meta-box-sortables ui-sortable">
					...
					<div id="mm-panel-primary" class="postbox">
						...
						<div class="toggle<?php if (!isset($_GET['settings-updated'])) echo ' default-hidden'; ?>">
							...
							<div class="mm-table-wrap">
								...
								<table class="widefat mm-table">
									...
									<tr>
										...
										<code>%%edit_link%%</code>, <code>%%delete_link%%</code>, <code>%%user_email%%</code>, <code>%%user_url%%</code>, <code>%%custom_field%%</code>, <code>%%custom_field_2%%</code>, <code>%%custom_checkbox%%</code>, <code>%%category%%</code>, <code>%%tags%%</code>, <code>%%ai_summary%%</code></div></td>
									</tr>
									<tr>
                            ...
                            <div class="mm-table-wrap">
                                <div class="mm-section-desc">
                                    <?php esc_html_e('Enable Cloudflare Turnstile CAPTCHA alternative.', 'usp'); ?>
                                    <a target="_blank" rel="noopener noreferrer" href="https://dash.cloudflare.com/?to=/:account/turnstile"><?php esc_html_e('Get your Turnstile keys here', 'usp'); ?></a>.
                                </div>
                                <table class="widefat mm-table">
                                    <tr>
                                        <th scope="row"><label class="description" for="usp_options[usp_turnstile_enable]"><?php esc_html_e('Enable Turnstile', 'usp'); ?></label></th>
                                        <td>
                                            <input type="checkbox" value="1" name="usp_options[usp_turnstile_enable]" <?php if (isset($usp_options['usp_turnstile_enable'])) checked('1', $usp_options['usp_turnstile_enable']); ?> />
                                            <span class="mm-item-caption"><?php esc_html_e('Enable Cloudflare Turnstile verification on the submission form.', 'usp'); ?></span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th scope="row"><label class="description" for="usp_options[usp_turnstile_site_key]"><?php esc_html_e('Turnstile Site Key', 'usp'); ?></label></th>
                                        <td>
                                            <input type="text" size="45" name="usp_options[usp_turnstile_site_key]" value="<?php if (isset($usp_options['usp_turnstile_site_key'])) echo esc_attr($usp_options['usp_turnstile_site_key']); ?>" />
                                            <div class="mm-item-caption"><?php esc_html_e('Enter your Cloudflare Turnstile Site Key (public).', 'usp'); ?></div>
                                        </td>
                                    </tr>
                                     <tr>
                                        <th scope="row"><label class="description" for="usp_options[usp_turnstile_secret_key]"><?php esc_html_e('Turnstile Secret Key', 'usp'); ?></label></th>
                                        <td>
                                            <?php
                                                // Basic obfuscation for display if key exists
                                                $secret_key_display = '';
                                                if (isset($usp_options['usp_turnstile_secret_key']) && !empty($usp_options['usp_turnstile_secret_key'])) {
                                                    $secret_key_display = esc_attr(substr($usp_options['usp_turnstile_secret_key'], 0, 4)) . str_repeat('*', max(0, strlen($usp_options['usp_turnstile_secret_key']) - 8)) . esc_attr(substr($usp_options['usp_turnstile_secret_key'], -4));
                                                }
                                            ?>
                                            <input type="text" size="45" name="usp_options[usp_turnstile_secret_key]" value="<?php if (isset($usp_options['usp_turnstile_secret_key'])) echo esc_attr($usp_options['usp_turnstile_secret_key']); ?>" placeholder="<?php echo $secret_key_display; ?>" />
                                            <div class="mm-item-caption"><?php esc_html_e('Enter your Cloudflare Turnstile Secret Key.', 'usp'); ?></div>
                                            <?php if (!empty($secret_key_display)) : ?>
                                                <!--<div class="mm-item-caption" style="font-style: italic;"><?php esc_html_e('Enter the full key to change it, or leave blank to keep the current one.', 'usp'); ?></div>-->
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                </table>
                            </div>
							<?php // --- END Cloudflare Turnstile --- ?>

                <h3 id="usp-ai-settings"><?php esc_html_e('AI Summary Settings', 'usp'); ?></h3>
                <div class="mm-table-wrap"> <!-- opens the new AI settings mm-table-wrap -->
                    <div class="mm-section-desc"><?php esc_html_e('Configure settings for the AI-powered summary generation using DeepSeek.', 'usp'); ?></div>
                    <table class="widefat mm-table">
                         <!-- ADD TOGGLE SWITCH -->
                        <tr>
                            <th scope="row"><label class="description" for="usp_options[ai_summary_enable]"><?php esc_html_e('Enable AI Summary', 'usp'); ?></label></th>
                            <td>
                                <input type="checkbox" value="1" name="usp_options[ai_summary_enable]" <?php if (isset($usp_options['ai_summary_enable'])) checked('1', $usp_options['ai_summary_enable']); ?> />
                                <span class="mm-item-caption"><?php esc_html_e('Generate an AI summary for submitted posts (requires DeepSeek API Key).', 'usp'); ?></span>
                            </td>
                        </tr>
                         <!-- END TOGGLE SWITCH -->

                        <tr>
                            <th scope="row"><label class="description" for="usp_options[deepseek_api_key]"><?php esc_html_e('DeepSeek API Key', 'usp'); ?></label></th>
                            <td>
                                <input type="text" size="45" name="usp_options[deepseek_api_key]" value="<?php if (isset($usp_options['deepseek_api_key'])) echo esc_attr($usp_options['deepseek_api_key']); ?>" />
                                <div class="mm-item-caption">
                                    <?php esc_html_e('Enter your API key from DeepSeek.', 'usp'); ?>
                                    <a target="_blank" rel="noopener noreferrer" href="https://platform.deepseek.com/api_keys"><?php esc_html_e('Get your key here', 'usp'); ?></a>.
                                    <!--<?php esc_html_e('The key will not be fully displayed after saving for security.', 'usp'); ?>-->
                                </div>
                            </td>
                        </tr>
                         ...
                         <!-- ADD PROMPT FIELDS -->
                         <tr>
                            <th scope="row"><label class="description" for="usp_options[ai_summary_system_prompt]"><?php esc_html_e('AI System Prompt', 'usp'); ?></label></th>
                            <td>
                                <textarea
                                    class="textarea"
                                    rows="5" cols="50"
                                    name="usp_options[ai_summary_system_prompt]"
                                    placeholder="<?php echo esc_attr($default_system_placeholder); // Use placeholder from helper ?>"
                                ><?php echo esc_textarea($system_prompt_value); ?></textarea>
                                <div class="mm-item-caption"><?php esc_html_e('The system instruction given to the AI model. Defaults will show as placeholder text if empty.', 'usp'); ?></div>
                            </td>
                        </tr>
                         <tr>
                            <th scope="row"><label class="description" for="usp_options[ai_summary_user_prompt]"><?php esc_html_e('AI User Prompt', 'usp'); ?></label></th>
                            <td>
                                <textarea
                                    class="textarea"
                                    rows="10" cols="50"
                                    name="usp_options[ai_summary_user_prompt]"
                                    placeholder="<?php echo esc_attr($default_user_placeholder); // Use placeholder from helper ?>"
                                ><?php echo esc_textarea($user_prompt_value); ?></textarea>
                                <div class="mm-item-caption"><?php esc_html_e('The specific task instruction given to the AI model. The post content will be appended after this prompt. Defaults will show as placeholder text if empty.', 'usp'); ?></div>
                            </td>
                        </tr>
                        <!-- END PROMPT FIELDS -->
                    </table>
                </div> <!-- closes the new AI settings mm-table-wrap -->
