...
# Custom Function to add cloudflare turnstile implementation
function verify_turnstile($token, $action = 'user-submitted-posts') {
    global $usp_options;
    
    $secret_key = isset($usp_options['usp_turnstile_secret_key']) ? trim($usp_options['usp_turnstile_secret_key']) : '';
    // --- Check if secret key is configured ---
    if (empty($secret_key)) {
        error_log('USP Turnstile Error: Secret Key not configured in settings.');
        return false; // Cannot verify without a secret key
    }
    ...
    if (is_wp_error($response)) {
        error_log('USP Turnstile Error: WP Error during verification - ' . $response->get_error_message()); // Log WP errors
		return false;
    }
    ...
    // Add more detailed logging for failure cases
    if (!(isset($data['success']) && $data['success'] === true)) {
        $error_codes = isset($data['error-codes']) ? implode(', ', $data['error-codes']) : 'N/A';
        error_log('USP Turnstile Verification Failed. Response Body: ' . $body . ' | Error Codes: ' . $error_codes);
    }
...
function usp_checkForPublicSubmission() {
	...
	if ($is_submitted && $is_allowed) {
        ...
        $turnstile_enabled = isset($usp_options['usp_turnstile_enable']) && $usp_options['usp_turnstile_enable'];

        if ($turnstile_enabled) {
            $turnstile_token = isset($_POST['cf-turnstile-response']) ? sanitize_text_field($_POST['cf-turnstile-response']) : ''; // Sanitize input
            $action = 'user-submitted-posts'; // Consider making this configurable if needed

            if (empty($turnstile_token) || !verify_turnstile($turnstile_token, $action)) {
                // Handle turnstile verification failure
                $redirect = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : ''; // Check if REQUEST_URI is set
                $redirect = remove_query_arg(array('success', 'post_id', 'usp-error'), $redirect);
                $redirect = add_query_arg(array('usp-error' => 'turnstile-failed'), $redirect); // Keep specific error

                // Ensure redirect URL is not empty before redirecting
                if (!empty($redirect)) {
                    wp_redirect(esc_url_raw($redirect));
                    exit();
                } else {
                    // Fallback if redirect URL is somehow empty
                    wp_die(esc_html__('Turnstile verification failed and redirect URL could not be determined.', 'usp'));
                }
            }
        }
...
function usp_prepare_post($title, $content, $excerpt, $author_id, $author, $ip) {
...
function usp_createPublicSubmission($title, $files, $ip, $author, $url, $email, $tags, $captcha, $verify, $content, $category, $custom, $custom_2, $checkbox, $comments) {
	...
	$postData = usp_prepare_post($title, $content, $custom, $author_id, $author, $ip);
	...
	if ($post_id && !is_wp_error($post_id)) {
		...
		if (isset($newPost['error']) && empty($newPost['error'])) {
			...
			usp_send_mail_alert($post_id, $title, $content, $author, $email, $url, $custom, $custom_2, $checkbox, $post_date, $tags, $category);
...
function usp_get_ai_summary($content) {
    global $usp_options;
	// Ensure options are loaded if not already global in this scope
    if (empty($usp_options)) {
        $usp_options = get_option('usp_options');
    }

    // --- Check if AI Summary is Enabled ---
    $ai_enabled = isset($usp_options['ai_summary_enable']) ? (bool) $usp_options['ai_summary_enable'] : false;
    if (!$ai_enabled) {
        return 'AI Summary generation is disabled in plugin settings.';
    }
    // --- End Enable Check ---
    ...
    $double_byte_character_count = preg_match_all('/[\x{4e00}-\x{9fff}]/u', $plain_text, $matches);

    // Count words in the content for single-byte characters submissions
    $word_count = str_word_count(preg_replace('/[\x{4e00}-\x{9fff}]/u', '', $plain_text));

    // Calculate Total Words:
    $total_word_count = $double_byte_character_count + $word_count;
    
    // Check if content is less than 500 words
    if ($total_word_count < 500) {
        return 'Content is too short for summarization (less than 500 words).';
    }

    // --- Get API Key from Options ---
    $api_key = isset($usp_options['deepseek_api_key']) ? trim($usp_options['deepseek_api_key']) : '';

    // --- Add check for empty API key ---
    if (empty($api_key)) {
        error_log('USP Plugin Error: DeepSeek API Key is not configured in settings.');
        return 'AI Summary Error: API key not configured in plugin settings.';
    }
    // --- End API Key retrieval ---

	// --- Get Prompts from Options OR use helper function for defaults ---
	$default_prompts = usp_get_default_ai_prompts(); // Call helper

	$system_prompt = isset($usp_options['ai_summary_system_prompt']) && !empty(trim($usp_options['ai_summary_system_prompt']))
					 ? $usp_options['ai_summary_system_prompt']
					 : $default_prompts['system']; // Fallback to helper default

	$user_prompt = isset($usp_options['ai_summary_user_prompt']) && !empty(trim($usp_options['ai_summary_user_prompt']))
				   ? $usp_options['ai_summary_user_prompt']
				   : $default_prompts['user']; // Fallback to helper default
    ...
    $attempt = 0;
    $error_log = array(); // Array to store all error messages
    ...
    while ($attempt < $max_retries) {
        ...
        try {
            ...
            $data = array(
                'model' => 'deepseek-chat', // Use deepseek-chat for general summarization
                'messages' => array(
                    array(
                        'role' => 'system',
            ...
            // Check for errors
            if (curl_errno($ch)) {
                $error_message = 'AI API Error: ' . curl_error($ch);
                error_log("Attempt $attempt failed: $error_message");
                $error_log[] = "Attempt $attempt: $error_message";
                curl_close($ch);
                ...
                return 'AI Summary Error: ' . implode("\n", $error_log);
            }
            ...
            if (isset($result['choices'][0]['message']['content'])) {
                ...
                $error_message = 'DeepSeek API Error: ' . $result['error']['message'];
                error_log("Attempt $attempt failed: $error_message");
                $error_log[] = "Attempt $attempt: $error_message";
                ...
                $error_message = 'Unknown error: Unable to generate summary';
                error_log("Attempt $attempt failed: $error_message");
                $error_log[] = "Attempt $attempt: $error_message";
            ...
            $error_message = 'Error generating AI summary: ' . $e->getMessage();
            error_log("Attempt $attempt failed: $error_message");
            $error_log[] = "Attempt $attempt: $error_message";
...
// modified usp_send_mail_alert function to schedule background processing
function usp_send_mail_alert($post_id, $title, $content, $author, $email, $url, $custom, $custom_2, $checkbox, $post_date, $tags, $category) {
    // Store necessary data as post meta for later retrieval
    update_post_meta($post_id, '_usp_temp_title', $title);
    update_post_meta($post_id, '_usp_temp_content', $content);
    update_post_meta($post_id, '_usp_temp_author', $author);
    update_post_meta($post_id, '_usp_temp_email', $email);
    update_post_meta($post_id, '_usp_temp_url', $url);
    update_post_meta($post_id, '_usp_temp_custom', $custom);
    update_post_meta($post_id, '_usp_temp_custom_2', $custom_2);
    update_post_meta($post_id, '_usp_temp_checkbox', $checkbox);
    update_post_meta($post_id, '_usp_temp_post_date', $post_date);
    update_post_meta($post_id, '_usp_temp_tags', $tags);
    update_post_meta($post_id, '_usp_temp_category', $category);
    
    // Schedule the background processing
    wp_schedule_single_event(time(), 'usp_process_ai_summary_and_email', array($post_id));
    
    return true; // Return immediately to allow redirect
}

// Create a new function to handle the background processing
function usp_process_ai_summary_and_email($post_id) {
    global $usp_options; // Ensure options are accessible
    ...
    // --- Check if AI Summary is Enabled Early ---
    $ai_enabled = isset($usp_options['ai_summary_enable']) ? (bool) $usp_options['ai_summary_enable'] : false;
    $ai_summary = ''; // Initialize summary variable

    if ($ai_enabled) {
        // Retrieve stored content ONLY if AI is enabled
        $content = get_post_meta($post_id, '_usp_temp_content', true);
        // Generate AI summary
        $ai_summary = usp_get_ai_summary($content);
        if (!empty($ai_summary) && strpos($ai_summary, 'AI Summary Error:') === false && strpos($ai_summary, 'AI Summary generation is disabled') === false) {
             // Check if ACF function exists before calling it
            if (function_exists('update_field')) {
                update_field('ai_summary', $ai_summary, $post_id);
            } else {
                // Fallback or alternative storage if ACF is not active
                update_post_meta($post_id, 'ai_summary', $ai_summary); // Example: store in regular post meta
                error_log('USP Plugin Notice: Advanced Custom Fields (ACF) plugin is not active. AI Summary stored in standard post meta.');
            }
        }
    } else {
        // If AI is disabled, set summary to a non-error message or leave empty
         $ai_summary = 'AI Summary generation is disabled.';
         $content = ''; // No need to retrieve content if AI is off
    }
    // --- End AI Check and Generation ---


    // Retrieve other stored data (needed for email regardless of AI status)
    $title = get_post_meta($post_id, '_usp_temp_title', true);
    // $content = get_post_meta($post_id, '_usp_temp_content', true); // Content already retrieved above if needed
    $author = get_post_meta($post_id, '_usp_temp_author', true);
    $email = get_post_meta($post_id, '_usp_temp_email', true);
    $url = get_post_meta($post_id, '_usp_temp_url', true);
    $custom = get_post_meta($post_id, '_usp_temp_custom', true);
    $custom_2 = get_post_meta($post_id, '_usp_temp_custom_2', true);
    $checkbox = get_post_meta($post_id, '_usp_temp_checkbox', true);
    $post_date = get_post_meta($post_id, '_usp_temp_post_date', true);
    $tags = get_post_meta($post_id, '_usp_temp_tags', true);
    $category = get_post_meta($post_id, '_usp_temp_category', true);
    
    if (isset($usp_options['usp_email_alerts']) && $usp_options['usp_email_alerts']) {

        // Make sure $post_content is retrieved if needed for the email template,
        // even if AI was disabled.
        if (empty($content) && strpos($usp_options['email_alert_message'], '%%post_content%%') !== false) {
            $content = get_post_meta($post_id, '_usp_temp_content', true);
        }
        $post_content = $content; // Assign for use in replacements
        
        $tags_string = implode(", ", $tags);
        $category_index = implode(", ", $category);
        
        if ($category_index == "5")      $category_string = "Gay";
        elseif ($category_index == "13") $category_string = "Straight";
        elseif ($category_index == "18") $category_string = "Bisexual";
        elseif ($category_index == "22") $category_string = "Female";
        elseif ($category_index == "29") $category_string = "None";
        elseif ($category_index == "91") $category_string = "Transgender";
        elseif ($category_index == "95") $category_string = "Any";
        else $category_string = "Unknown";
        ...
        // $post_content = $content;                // %%post_content%%  // Already assigned above
        $post_author  = $author;                    // %%post_author%%
        $user_email   = $email;                     // %%user_email%%
        $user_url     = $url;                       // %%user_url%%
        $post_tags    = $tags_string;               // %%tags%%
        $post_category= $category_string;           // %%category%%
        ...
        $patterns[14] = "/%%tags%%/";
        $patterns[15] = "/%%category%%/";
        $patterns[16] = "/%%ai_summary%%/";
        $patterns[17] = "/%%custom_checkbox%%/";
        ...
        $replacements[14] = $post_tags;
        $replacements[15] = $post_category;
        $replacements[16] = $ai_summary;
        $replacements[17] = $checkbox;
    ...
    // Clean up temporary meta data
    delete_post_meta($post_id, '_usp_temp_title');
    delete_post_meta($post_id, '_usp_temp_content');
    delete_post_meta($post_id, '_usp_temp_author');
    delete_post_meta($post_id, '_usp_temp_email');
    delete_post_meta($post_id, '_usp_temp_url');
    delete_post_meta($post_id, '_usp_temp_custom');
    delete_post_meta($post_id, '_usp_temp_custom_2');
    delete_post_meta($post_id, '_usp_temp_post_date');
    delete_post_meta($post_id, '_usp_temp_tags');
    delete_post_meta($post_id, '_usp_temp_category');
}

// Register the action hook
add_action('usp_process_ai_summary_and_email', 'usp_process_ai_summary_and_email');
...
function usp_error_message() {
	...
	if (isset($_GET['usp-error']) && !empty($_GET['usp-error'])) {
		...
		foreach ($error_array as $e) {
			...
			elseif ($e == 'required-title')      $error[] = esc_html__('Story title required', 'usp');
			elseif ($e == 'required-url')        $error[] = esc_html__('User URL required', 'usp');
			elseif ($e == 'required-tags')       $error[] = esc_html__('Stpru tags required', 'usp');
			elseif ($e == 'required-category')   $error[] = esc_html__('Story category required', 'usp');
			elseif ($e == 'required-content')    $error[] = esc_html__('Story content required', 'usp');
			...
			elseif ($e == 'turnstile-failed')    $error[] = esc_html__('Please complete the Turnstile verification', 'usp');
			...
			elseif ($e == 'post-fail')           $error[] = esc_html__('Story not submitted. Please contact the site administrator for help.', 'usp');
			elseif ($e == 'duplicate-title')     $error[] = esc_html__('Duplicate story title. Please try again.', 'usp');
